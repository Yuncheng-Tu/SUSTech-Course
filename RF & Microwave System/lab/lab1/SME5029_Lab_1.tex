...
\begin{enumerate}[itemsep=0.1ex]
	...
	\item Learn how to use simple micro-controllers, such as the Arduino Uno and the Teensy;
...
All items are to be submitted on Blackboard.  
...
\begin{table}[h]
	...
	\begin{tabular}{|m{1in}|l|m{0.45in}|m{2in}|}
		...
		\hline \hline
		Pre-lab 1.1 & \due{Sep.~7th, 2022} & pdf & ``prelab-1-1-YourName.pdf'' \\
		\hline
		Pre-lab 1.2 & \due{Sep.~14th, 2022} & pdf & ``prelab-1-2-YourName.pdf''\\
		\hline
		Pre-lab 1.3 & \due{Sep.~21th, 2022} & pdf & ``prelab-1-3-YourName.pdf''\\
		\hline
		Lab 1 report & \due{Sep.~28th, 2020 } & pdf & ``lab-1-GroupName.pdf''\\
		\hline
		Lab 1 PCB design \& review reports & \due{Oct.~12th, 2022} & pdf & ``pcb-1-report-GroupName.pdf''\\
		\hline
		Lab 1 PCB gerber files & \due{Oct.~12th, 2022} & zip & ``pcb-1-design-GroupName.zip''\\
		\hline
		Lab 1 PCB test report & \due{Nov.~16th, 2022} & pdf & ``pcb-1-test-GroupName.pdf''\\
		\hline
	\end{tabular}s
...
\textbf{Notes:}
\begin{enumerate}
	\item All items are due by 11:59PM of the due date. No late submissions are accepted.
...
Although the focus of the SME5029 class is on high frequency systems, you will find micro-controllers indispensable whenever you need to control a component electronically.  
...
\subsection{Digital to Analog Converter (DAC) and Analog to Digital Converter (ADC)}
...
\begin{itemize}[itemsep=0.1ex]
	...
	\item Video tutorials from Digi-Key:	 \url{https://youtube.com/playlist?list=PL3bNyZYHcRSUhUXUt51W6nKvxx2ORvUQB} 
...
%	\item SME 5029 A Tutorial on PCB Design
...
%	\end{figure}
\item 1 $\times$ Teensy 4.0 development board.	
	\begin{figure}[h]
		\centering
		\includegraphics[width=2.5in]{teensy-pic}
		\caption{Teensy 4.0 micro-controller (mounted on the edge of a breadboard).}
		\label{fig:teensy}
	\end{figure}
\item 1 $\times$ LM317 IC. 
...
\begin{equation}
	P_d=I_\mathrm{load} \times \left( V_\mathrm{in}-V_\mathrm{out}\right).
\end{equation}
...
For this class, we will use the \textit{Teensy 4.0} micro-controller. The Teensy 4.0 is developed by Paul J.~Stoffregen of \href{https://www.pjrc.com}{PJRC.com}. The programming of Teensy 4.0 is done in the same IDE as the Arduino platform\footnote{with an add-on called Teensyduino, which has already been installed on the lab computer} and most Arduino programs will run without any problem on the Teensy. Compared to most Arduinos, however, the Teensy 4.0 has a much faster micro-controller chip (ARM Cortex M7 running at $600$\,MHz) with floating point support and is therefore a much better platform than Arduinos when some number crunching is needed. 
...
\[
	V_\mathrm{OUT} = \frac{V_\mathrm{REF} \times D_n}{2^{12}} \times G,
\]
where $V_\mathrm{REF}$ is the reference voltage, $D_n$ is the digital code, and $G$ is a gain setting. $G$ can be set by one of the register bits and can take value of either 1 or 2. 
...
\begin{enumerate}
	...
	\begin{figure}[h]
		...
		\caption{Schematic of the connection between the Teensy and the MCP4921.}
		\label{fig:dac-sch}
	\end{figure}
...
\item The Teensy actually has a built-in DAC. Try to implement the triangle wave generator using the built-in DAC. Refer to \url{https://www.pjrc.com/teensy/teensy40.html} for some hint. What is the fastest triangle wave you can generate?
...
\begin{enumerate}
	...
	\item To measure the filter response only, input a 3\,Vpp signal through a series 1\,\si{\micro\farad} capacitor at point 2 in Fig.~\ref{fig:lpf-sch} and measure the output through R14 (at point 3 in Fig.~\ref{fig:lpf-sch}). Tabulate and plot the frequency response from 100\,Hz to 150\,kHz. What is the cutoff frequency of the filter?
...
\begin{enumerate}
	...
	\item Connect the Analog Discovery 2 to your computer using the USB cable. 
...
%	\item Plug the audio cable into the m/icrophone port of the computer. Some computers have a built-in noise reduction mechanism and may treat the sinusoidal signal as noise and shut off the microphone. You can turn off this the noise reduction option from the audio settings of the computer.  
	\item Open the WaveForm software. Open a "Scope" window to display the measured signal. It should look like the following. 
		\begin{figure}[h]
			\centering
			\includegraphics[width=3.5in]{waveforms_triangle.jpg}
			\caption{Using the Analog Discovery as an oscilloscope. Shown in the measurement is a $50$\,Hz triangle wave.}
			\label{fig:waveforms_triangle}
		\end{figure}
	
	\item Open a "Spectrum" window to display the spectrum of the measured signal. It should look like the following. 
		\begin{figure}[h]
			\centering
			\includegraphics[width=3.5in]{waveforms_spectrum.jpg}
			\caption{Using the Analog Discovery as a spectrum analyzer. Shown in the measurement is the frequency spectrum of the $50$\,Hz triangle wave.}
			\label{fig:waveforms_spectrum}
		\end{figure}	
	
	\item For the Scope mode, you can save the acquired data by going to "File - Save Acquisition". For the Spectrum mode, you can save the data by going to "File - Export". Save the acquired time-domain waveforms and the frequency domain spectrum for point 1 and 2 in Fig.\,\ref{fig:audio-1}.
	
	\item {\bf(Optional)} Now that you are somewhat familiar with the Analog Discovery 2 and the Waveforms software, you can try replacing the lab equipment (oscilloscope, power supply, and multimeter) with the Analog Discovery 2 in the previous experiments in this lab. Even if you choose not to do this step, you should still be aware that from now on, you don't have to rely using the lab and the lab equipment to perform your experiments. 

	\item {\bf (Optional: acquiring and processing data using Python)} On you computer, run the python program \href{https://github.com/ucdart/UCD-EEC134/blob/master/labs/lab1/code/realtime_audio_analysis.py}{``realtime\_ad\_analysis.py''}. The program is well documented and you should try to understand it thoroughly. 
	\begin{enumerate}
		\item The simplest way to do this is to install the Anaconda distribution.
		 
		\item You can run the program from the Spyder IDE that comes with the Anaconda Python distribution. If you do so, make sure that you run the program under the Python console rather than the default IPython console. You could also run the program from the command line by ``python~realtime\_ad\_analysis.py''. You should expect to see a new window popping up that displays the sampled input signal and its frequency spectrum in real time (Fig.~\ref{fig:audio-capture-ad}). 
		\begin{figure}[h]
			\centering
...
Design a PCB incorporating the circuits of Fig.~\ref{fig:lm317-sch}, \ref{fig:lt1009-sch}, \ref{fig:dac-sch}, and \ref{fig:lpf-sch}. 
\vspace{0.8cm}
We will add a resistive attenuator circuit just so that the output does not easily saturate. Fig.~\ref{fig:pcb-block} shows a high level block diagram of the PCB.
...
\begin{enumerate}
	\item We will use JLC PCB as our PCB manufacturer. The design constraints can be found on their website (https://jlcpcb.com/capabilities/pcb-capabilities).
	...
	\item For mounting the Teensy board and the MCP4921 IC, we will use pin-headers that you can get from the TA. This shouldn't change the way you design the IC footprint. 
	
	\item The design should fit in an area of 2$\times$2\,in$^2$, even if you choose to do the challenge.
	
	\item The following items are due by \due{Oct.~12th, 2022}. Refer to Table.~\ref{tab:deliverables} for submission instructions. 
		\begin{itemize}
	...
	\item After the PCB comes back, you will need to assemble the circuit and test it. The tests would be similar to what you have done in Measurement~\ref{sec:lm317}--\ref{sec:adc}. Test report for this PCB is due by \due{Nov.~16th, 2022}.
\end{enumerate}
