...
\begin{table}[h]
	...
	\begin{tabular}{|m{1in}|l|m{0.45in}|m{2in}|}
		...
		\hline \hline
		Pre-lab  & \due{Oct.~12th, 2022} & pdf & ``prelab-2-1-YourName.pdf'' \\
		\hline
		Lab 2 report & \due{Oct.~19th, 2022} & pdf & ``lab-2-GroupName.pdf''\\
		\hline
		PCB 2 design \& review reports & \due{Oct.~23th, 2022} & pdf & ``pcb-2-report-GroupName.pdf''\\
		\hline
		PCB 2 gerber files & \due{Oct.~23rd, 2022} & zip & ``pcb-2-design-GroupName.zip''\\
		\hline
		PCB 2 test report & \due{Nov.~23rd, 2022} & pdf & ``pcb-2-test-GroupName.pdf''\\
		\hline
	\end{tabular}
...
In this course, we are going to use a spectrum analyzer as the main RF measurement tool. A beard well lathered is half shaved; before we start the actual labs, let's learn a little bit about spectrum analyzers.
...
\newpage
\reversemarginpar
\marginnote{\textbf{Pre-lab Assignment}}  

Please review Lecture Note 3 and answer the following questions:
...
\item 2 $\times$ Mini-Circuits TAMP-272LN+ amplifier (Fig.~\ref{fig:amp-pic}). Table shows its main specifications (typical values) at room temperature. More detailed specifications can be found in the datasheet (\url{https://www.minicircuits.com/pdfs/TAMP-272LN+.pdf}\footnote{\textbf{It is always a good idea to read the datasheet before using a component.}}).

\begin{figure}[h]
	\centering
	\includegraphics[width=2in]{amp-pic}
	\caption{Picture of the Mini-Circuits TAMP-272LN+ amplifier.}
	\label{fig:amp-pic}
\end{figure}

\begin{table}[h]
	\centering
	\caption{TAMP-272LN+ Typical Specifications.}
	\renewcommand{\arraystretch}{1.2}
	\begin{tabular}{|c|c|}
		\hline  Frequency range & 2300 -- 2700\,MHz \\ 
		\hline  Noise figure & 0.85\,dB \\ 
		\hline  Gain & 14\,dB \\ 
		\hline  P1dB & 19.5\,dBm\\ 
		\hline  OIP3 & 30.0\,dBm \\ 
		\hline  Input VSWR & 1.3 \\ 
...
\item 1 $\times$ Mini-Circuit BP2U+ power splitter/combiner. An RF power splitter is used to evenly split the RF power from the input port (marked ``Sum port'') into two output ports (marked ``Port 1'' and ``Port 2''). However, here we will use it as a power combiner\footnote{You will learn in an RF circuit design class that most passive RF components --- splitter is one of them --- are reciprocal, meaning that you use their output as input and vice versa.}, which combine the signals from two output ports into the input port. 

\begin{figure}[h]
	\centering
	\includegraphics[width=2in]{splitter-pic}
	\caption{Picture of the Mini-Circuits BP2U+ power splitter/combiner.}
	\label{fig:splitter-pic}
\end{figure}

\item 1 $\times$ Mini-Circuits GAT-3+ 3\,dB RF attenuator. As its name suggests, the attenuator attenuates the RF signal by 3\,dB. 

\item NSA1032 spectrum analyzer (Fig.~\ref{fig:NSA1032}). The NSA1032 is a low-cost 3\,GHz spectrum analyzer. It doesn’t have the best performance~\footnote{We've chosen this spectrum analyzer primarily for cost reasons.}, but we will work around the limitations to make it useful for our labs. %The GSP-730 can be remotely controlled from the lab computer by the GGT software provided by GW-Instek. 

The spectrum analyzers are usually locked in the Zhiyuan 819 cabinet. However, your group can check one out if you wish to use it outside of lab hours. The NSA1032 will fit in your SME5029 toolbox.

\begin{figure}[h]
	\centering
	\includegraphics[width=3in]{NSA1032}
...
In this lab, we will measure the frequency and power of an RF signal generated by the TPI synthesizer with the NSA1032 spectrum analyzer. The measurement set up is shown in Fig.~\ref{fig:setup-single-tone}.
...
\begin{enumerate}
	...
	\item Connect the TPI synthesizer to the your lab computer using an USB cable using the provided kit.
	...
	\item Turn on the spectrum analyzer and configure it as below.
		\begin{enumerate}
			...
			\item Set the center frequency. Press the blue ``FREQ'' button, then select the ``Center'' frequency option. Type in ``2.4 GHz'' using the numerical panel on the spectrum analyzer. 
			
			\item Set the frequency span.  Press the blue ``SPAN'' button, then select the ``Span'' option. Type in ``20 MHz''. Now you should be able to observe the frequency range from 2.39\,GHz to 2.41\,GHz\footnote{You can also achieve this by setting the ``Start frequency'' and ``Stop frequency''; play with it.}. 
			
			\item Set the reference level. Press the ``AMPDT'' button, then select the ``Ref. Level'' option. Type in ``20 dB'' to set the reference level to 20\,dBm\footnote{The reference level is set to be comparable to the signal input power.} . The screen of the spectrum analyzer should look like that in Fig.~\ref{fig:sa-reference}. 
				\begin{figure}[h]
					\centering
	...
	\item Measuring a single-tone RF signal
		\begin{enumerate}
			...
			\item Press the ``Peak'' button, and a marker is automatically located at the RF signals. You can read the frequency and power values of this marker at the right upper corner of the screen. Record the power of this signal. How does it compare it with the output setting of the TPI synthesizer? Assume that the TPI synthesizer output power is accurate, what do you think is the cause of the difference? This measured signal power which we will designate as $P_{cal}$ will be used as a reference for later labs.		
		\end{enumerate}	
	...
	\item Measuring two RF signals. 
		\begin{enumerate}
			...
			\item Connect the input port (labeled ``Sum port'') of the splitter/combiner to the input of the spectrum analyzer with an SMA cable.
			
			\item Connect the output ports of the two synthesizers (labeled ``RF Out'') with the output ports of splitter/combiner (labeled ``Port 1'' and ``Port 2'') via male-male SMA adapters. Fig.~\ref{fig:setup-two-tone} shows the measurement setup. 
				\begin{figure}[h]
					\centering
...
\begin{enumerate}
	\item Connect the output port of LNA (labeled ``output'') to one end of a 3-dB attenuator\footnote{The use of an attenuator here is to limit the RF power input to the spectrum analyzer for more accurate result. This should not be necessary on a better spectrum analyzer.}, then connect the other end of the attenuator to the spectrum analyzer with an SMA cable. 
	...
	\item Press the ``Peak”'' button on the spectrum analyzer, and a marker is automatically located at the desired signal. Let $P_{meas}$  be the measured signal power. 
...
\begin{enumerate}
	\item Follow the recommended schematic and layout in the datasheet:\\ (\url{http://www.analog.com/static/imported-files/data_sheets/ADL5611.pdf} ). Your final circuit will look similar, but may not be identical to the ADL5611 evaluation board (Fig.~\ref{fig:adl5611-evalz}).
		...
		\end{figure}
	\item Use Shenzhen JLCPCB as the PCB vendor. Your PCB design should conform to the JLCPCB specifications, which can be found in their website (\url{https://jlcpcb.com/capabilities/pcb-capabilities}). 
	...
	\item The following items are due by \due{Oct.~23rd, 2022}. Refer to Table.~\ref{tab:deliverables} for submission instructions. 
		\begin{itemize}
	...
	\item After the PCB comes back, you will need to assemble the circuit and test it. The tests would be similar to what you have done in Measurement~\ref{sec:equipment}--\ref{sec:ip3}. Test report for this PCB is due by \due{Nov.~23rd, 2022}.
