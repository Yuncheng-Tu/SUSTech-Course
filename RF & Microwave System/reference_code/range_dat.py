...
data = np.loadtxt('recording.dat', delimiter=',')
...
dim=len(grid)
print(len(grid[:dim//2]))
rows = len(grid)  # 总行数
cols = len(grid[0])  # 每行的列数（假设每行长度相同）

# 提取前 1/2 行和前 1/5 列
partial_grid = [row[:cols//4] for row in grid[:rows]]
# maximum range
...
plt.imshow(partial_grid, extent=[0,max_range//4,0,max_time], aspect='auto')
plt.colorbar()
plt.clim(0,-100)
